...
import streamlit as st, pdfplumber, re, io, pandas as pd
    ...
    with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
        for page in pdf.pages:
            ...
            for raw_line in text.splitlines():
                ...
                if m_order: current_order_digits = m_order.group(1)
                mpn = None
                for patt in rules.get("mpn_patterns", []):
                    m = re.search(patt, line)
                    if m: mpn = m.group(1); break
                if not mpn: continue
                qty, total = None, None
                ...
                for patt in rules.get("total_patterns", []):
                    m = re.search(patt, line)
                    if m:
                        try: total = round(float(m.group(1).replace(",", ".")), 2)
                        except: pass
...
if pdf_file:
    ...
    if st.button("Скачать waybill"):
        if tpl_file:
            wb, ws = load_workbook(tpl_file), load_workbook(tpl_file).active
        else:
            wb, ws = Workbook(), Workbook().active
            ws["A1"]="MPN"; ws["B1"]="Replacem"; ws["C1"]="Quantity"; ws["D1"]="Totalsprice"; ws["E1"]="Order reference"
        for r in range(2, 1000):
            for c in range(1,6): ws.cell(row=r, column=c, value=None)
        for i, row in enumerate(edited.values.tolist(), start=2):
            for j, value in enumerate(row, start=1):
                ws.cell(row=i, column=j, value=value)
        bio = io.BytesIO(); wb.save(bio)
        st.download_button("Скачать waybill.xlsx", data=bio.getvalue(), file_name="waybill.xlsx")
